...
describe('UserRequest', () => {
  ...
  beforeAll(async () => {
    ...
    const query = /* GraphQL */ `
      mutation signInAdmin {
        signInAdmin(email: "poubelle+1@totem.paris", password: "totem") {
          token
        }
      }
    `
    token = await request(app.getHttpServer())
      ...
      .send({ query: query })
      .then(response => {
        return response.body.data.signInAdmin.token
      })
  })

  const generateQuery = (first = 8, after = '', type = 'requests') => {
    const query = /* GraphQL */ `
    query ${type} {
      ${type}(
        first: ${first}
  ...
  it(`QUERY requests`, async () => {
    const query = generateQuery(8, '')
    const queryName = 'requests'
